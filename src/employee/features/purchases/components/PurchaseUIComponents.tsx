    ...
    >
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] font-mono font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800/40">
              {purchase.billNumber || purchase.invoiceNumber}
            </span>
            {purchase.invoiceNumber && purchase.invoiceNumber !== purchase.billNumber && (
              <span className="text-[10px] font-mono text-zinc-600 dark:text-zinc-400 bg-gray-100 dark:bg-neutral-800 px-1.5 py-0.5 rounded">
                Inv: {purchase.invoiceNumber}
              </span>
            )}
            {purchase.items && purchase.items.length > 1 && (
              <span className="text-[10px] font-mono font-bold text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/40 px-1.5 py-0.5 rounded border border-purple-200 dark:border-purple-800/40">
                {purchase.items.length} Fabrics
              </span>
            )}
      ...
      <div className="flex justify-between items-center border-t border-gray-100 dark:border-neutral-800/60 pt-2.5 mt-2">
        <div className="text-xs font-mono text-gray-500 dark:text-neutral-400">
          Qty: <strong className="text-gray-800 dark:text-neutral-200">{qtyText}</strong> {purchase.items && purchase.items.length > 1 ? `(${purchase.items.length} items)` : (purchase.width ? `(${purchase.width})` : '')}
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1 bg-gray-100/70 dark:bg-neutral-900/90 p-0.5 rounded-xl border border-gray-200/60 dark:border-neutral-800/80">
                {['All', 'Paid', 'Pending', 'Partial'].map((pf) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer ${paymentFilter === pf
            ...
            {onWarehouseFilterChange && (
              <div className="relative flex items-center">
                <div
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-xl border text-xs font-mono transition-all ${warehouseFilter !== 'All'
                ...
                >
                  <Building2
                    className={`w-3.5 h-3.5 shrink-0 ${warehouseFilter !== 'All'
