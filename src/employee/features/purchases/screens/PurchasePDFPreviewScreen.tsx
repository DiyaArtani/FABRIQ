              ...
              <div className="border border-zinc-200 rounded-lg overflow-hidden">
                ...
                <div className="divide-y divide-zinc-150">
                  {(purchase.items && purchase.items.length > 0 ? purchase.items : [{
                    ...
                    <div key={item.id || idx} className="px-4 py-3 flex justify-between items-center text-xs text-zinc-800">
                      <div className="w-2/5 flex flex-col gap-0.5">
                        <span className="font-bold text-zinc-900">{item.fabricName}</span>
                        <span className="text-[10px] text-zinc-400">Stored at: {purchase.warehouse || purchase.warehouseLocation}</span>
                      </div>
                      <span className="w-1/5 text-center font-semibold text-zinc-600">{item.width || purchase.width || '58"'}</span>
                      <span className="w-1/5 text-right font-semibold font-mono">{(item.meters || 0).toLocaleString()} m</span>
                      <span className="w-1/5 text-right font-semibold font-mono">₹{(item.rate || 0).toFixed(2)}</span>
